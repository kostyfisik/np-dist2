...
shutil.rmtree('src/np_dist2.egg-info', ignore_errors=True);
for f in glob.glob('src/np_dist2/*.so'): os.remove(f);
for f in glob.glob('src/np_dist2/*.c'): os.remove(f);
"""
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"NP_DIST2_COMPILE_MYPYC": "1"})
