...
def main():
    ...
    # Filter out auxiliary files generated by other analysis scripts
    excluded_suffixes = ['_densities.npy', '_lattice_params.npy', '_radial_distances.npy']
    npy_files = [f for f in npy_files if not any(f.name.endswith(s) for s in excluded_suffixes)]

    if not npy_files:
        print(f"Error: No valid atomic coordinate .npy files found in '{input_dir}'", file=sys.stderr)
        sys.exit(1)
