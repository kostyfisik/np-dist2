...
def core_from_experimental_density(rho_exp_angs: float, factor: float,
    ...
    """
    A, B = _geometry_factors(width, r_max)
    return (rho_exp_angs * r_max**3 / 3 - factor * B) / A
...
def main():
    ...
    for ds in fit["datasets"]:
        ...
        if stem not in exp_densities:
            ...
            continue
        rho_exp_angs = exp_densities[stem] * EXP_DENSITY_SCALE
        core_new = core_from_experimental_density(
            rho_exp_angs, ds["indiv_factor"], ds["indiv_width"], ds["r_max"]
        )
        n_new = analytical_atom_count(
            ds["indiv_factor"], core_new, ds["indiv_width"], ds["r_max"]
        )
        actual = actuals[stem]
        err_new = (n_new - actual) / actual * 100
        print(f"  {ds['filename']:<30} {rho_exp_angs:>12.8f}  "
              f"{ds['indiv_core']:>10.6f} {core_new:>10.6f}  "
              f"{n_new:>10.1f} {err_new:>+9.2f}%")
    ...
    if has_combined:
        ...
        for ds in fit["datasets"]:
            ...
            rho_exp_angs = exp_densities[stem] * EXP_DENSITY_SCALE
            core_new = core_from_experimental_density(
                rho_exp_angs, fit["combined_factor"], fit["combined_width"], ds["r_max"]
            )
            n_new = analytical_atom_count(
                fit["combined_factor"], core_new, fit["combined_width"], ds["r_max"]
            )
            actual = actuals[stem]
            err_new = (n_new - actual) / actual * 100
    ...
    print("  core_new             : analytically solved to satisfy ρ_avg = ρ_exp")
